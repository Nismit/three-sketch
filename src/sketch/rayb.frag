...
float getDistance(vec3 p, float size) {
  ...
  vec3 p1 = sphericalPolarCoord(
    sin(PI * (time/300.0)) * 1.0,
    cos(PI * (time/300.0)) * -1.5,
    cos(PI * (time/300.0))
  );
  float d1 = sphere(p + p1, size);

  vec3 p2 = sphericalPolarCoord(
    sin(PI * (time/300.0)) * 0.55,
    cos(PI * (time/300.0)) * 1.528,
    sin(PI * (time/300.0))
  );
  float d2 = sphere(p + p2, size);

  vec3 p3 = sphericalPolarCoord(
    sin(PI * (time/300.0)) * -1.528,
    sin(PI * (time/300.0)) * -0.3,
    sin(PI * (time/300.0)) * 0.82
  );
